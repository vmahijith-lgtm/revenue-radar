...
PROJECT_ROOT = Path(__file__).resolve().parent.parent  # attribution_engine/
DB_PATH = PROJECT_ROOT / "ATTRIBUTION_ENGINE" / "attribution_project" / "dev.duckdb"
...
def get_available_models(con):
    models = {
        ...
        # 🔹 New: ROI Comparison (all models, all channels)
        "ROI Comparison": {
            "available": False,
            "table": "roi_attribution",
            "col": None,  # special handling
        },
    }
...
if selected_model_name == "Model Comparison":
    # Existing comparison of revenue by model
    query = """
        SELECT
            channel,
            val_first_touch,
    ...
    # NEW: ROI% comparison across models
    query = """
        SELECT
            channel,
            roi_first_touch,
    ...
    # Single-model views: revenue or ROI or whatever col the config specifies
    query = f"""
